...
void BSTreeLevelOrder(BSTree t) {
	...
	while(!QueueIsEmpty(q)) {
		BSTree find = QueueDequeue(q);
		printf("%d ", find -> value);
		if(find -> left != NULL)
			QueueEnqueue(q, find -> left); 
		if(find -> right != NULL)
			QueueEnqueue(q, find -> right);
	}
	free(q);
}
////////////////////////////////////////////////////////////////////////
...
int BSTreeNumLeaves(BSTree t) {
	if(t == NULL) return 0;
	return (t -> left == NULL && t -> right == NULL) ? 1 : 0 + BSTreeNumLeaves(t -> left) + BSTreeNumLeaves(t -> right);
}
