...
bool ScheduleAdd(Schedule s, Time t) {
    ...
    Time min = TimeSubtractMinutes(t, 10);
    Time max = TimeAddMinutes(t, 10);
    Time floor = TreeFloor(s->times, max);
    Time ceiling = TreeCeiling(s->times, min);
    if(floor != NULL && ceiling != NULL)
        if(TimeCmp(ceiling, max) < 0 || TimeCmp(floor, min) > 0) {
            return false;
        }
