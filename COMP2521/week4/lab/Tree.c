...
Record upper, List l) {
    ...
    // normal case
    // compare to our lower and upper, find range.                             
    int cmp_min = t -> compare(lower, n -> rec);
    int cmp_max = t -> compare(upper, n -> rec);
    // if current node less than lower, stop search.
    if (cmp_min < 0) {
        doTreeSearchBetween(t, n ->left, lower, upper, l);
    }
    // using 'inder' for lower to upper.
    if (cmp_min <= 0 && cmp_max >= 0) {
        ListAppend(l, n -> rec);
    }
    // if current node greater than upper, stop search.
    if (cmp_max > 0) {
